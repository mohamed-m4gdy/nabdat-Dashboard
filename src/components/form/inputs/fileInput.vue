...
const props = defineProps(['value', 'imageWrapper', 'label', 'items', 'index', 'minus', 'link'])
const emit = defineEmits(['update:value', 'removeIndex', 'update:link'])
...
const upladFile = (event: any) => {
  ...
  axios.post(linkGnirator('media/add'),
    ...
    emit('update:link', response.data.data[0].url)
