...
mock.onGet('/apps/invoices').reply(config => {
  ...
  const { sortBy = '', page = 1, per_page = 10 } = options
  ...
  return [200, { invoices: paginateArray(filteredInvoices, per_page, page), totalInvoices, page: page > Math.ceil(totalInvoices / per_page) ? 1 : page }]
})
