...
mock.onGet('/apps/users/list').reply(config => {
  ...
  const { sortBy = '', per_page = 10, page = 1 } = options
  ...
  return [200, { users: paginateArray(filteredUsers, per_page, page), totalPages, totalUsers, page: page > Math.ceil(totalUsers / per_page) ? 1 : page }]
})
