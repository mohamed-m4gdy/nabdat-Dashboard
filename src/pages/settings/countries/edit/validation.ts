...
const rules = {
  ...
  code: {
    ...
    minLength: helpers.withMessage('Code must be bigger than or equal 2', minLength(2)),
  },
  currency_id: { required: helpers.withMessage('Currency is required', required) },
  lat: { required: helpers.withMessage('Lat is required', required) },
  lon: { required: helpers.withMessage('Lon is required', required) },
}
