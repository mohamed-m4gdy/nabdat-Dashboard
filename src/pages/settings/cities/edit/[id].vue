...
const formSetup: Iform = {
  ...
  lon: null,
  status: true,
  home_visit_allowed: false,
  government: {
    name: '',
  },
}

const { item, loading, getInputError, saveItem, serverErrors } = formHock<Iform>('city', 'edit', formSetup, validation, parseInt(route.params.id as string))
        ...
        <VRow>
          ...
          >
            <Search
              ...
              :label-string="$t('Add governorate to this city')"
              paths="government"
          ...
          <VCol cols="6">
            <VCheckbox
              v-model="item.home_visit_allowed"
              :label="$t('Home Visit')"
            />
          </VCol>
        </VRow>
