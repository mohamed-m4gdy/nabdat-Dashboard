          ...
          <VRow class="mb-2">
            <VCol cols="12" :md="items.status !== 'approved' ? 6 : 12">
              <VCard class="py-3 px-5 h-100">
                <h4 class="mb-5">{{ $t('Workspace information') }}</h4>
                <div class="d-flex flex-column gap-2">
                  ...
                  <div class="d-flex gap-1">
                    <h4>{{ $t('Doctor') }}:</h4>
                    <div>{{ items.object?.doctor?.name }}</div>
                  </div>
                  <div>
                  ...
                  <div class="d-flex gap-1">
                    <h4>{{ $t('Cost') }}:</h4>
                    <div>{{ items.object?.cost }}</div>
                  </div>
                  <div class="d-flex gap-1">
                    <h4>{{ $t('Follow up cost') }}:</h4>
                    <div>{{ items.object?.follow_up_cost }}</div>
                  </div>
                  <div class="d-flex gap-1">
                    <h4>{{ $t('Waiting time') }}:</h4>
                    <div>{{ items.object?.waiting_time }}</div>
                  </div>
                  <div class="d-flex gap-1">
                    <h4>{{ $t('Lat and lon') }}:</h4>
            ...
            <VCol cols="12" md="6">
              <EditForm :items="items" v-if="items.status !== 'approved'"/>
            </VCol>
          </VRow>
          <div class="mb-5">
