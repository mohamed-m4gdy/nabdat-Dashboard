...
const formSetup: Iform = {
  branch_id: route.params.branchId as any as number,
  title: {
    ar: '',
    en: '',
  },
...
// use less fex just one time

const { items } = tableHock('WorkspaceCategory', false)

watch(items, value => {
  if (value.length)
    item.value.category_id = value[0].id
})
</script>
