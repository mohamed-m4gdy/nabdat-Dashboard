...
import { isAtLeast18YearsOld } from '@/helpers/dateValidation'
...
const rules = {
  ...
  dob: {
    ...
    isAtLeast18YearsOld: helpers.withMessage('The date of birth must be at least 18 years ago and at most 100 years ago.', isAtLeast18YearsOld),
  ...
  ratio: {
    call: {
      price_type: {
        required: helpers.withMessage('Call type is required', required),
      },
      value: {
        required: helpers.withMessage('Call ratio is required', optionalValidation),
        numeric: helpers.withMessage('Call ratio must be number', numeric),
        minValue: helpers.withMessage('Call ratio must be bigger than or equal 1', minValue(1)),
        maxValue: helpers.withMessage('Call ratio must be smaller than or equal 100', maxValue(100)),
      },
    },
    visit: {
      price_type: {
      ...
      value: {
        required: helpers.withMessage('Visit ratio is required', optionalValidation),
