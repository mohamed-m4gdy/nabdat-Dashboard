...
const {
  ...
  addFilter,
  resetFilter,
} = tableHock('support/doctor/request')
...
const { item, loading: showLoading, show, close, showDialog } = showHock('support/doctor/request/')
  ...
  <div>
    <Breadcrubs
      :links="[
        ...
        {
          title: $t('Support doctors'),
        }
      ]"
    />
    <PageHeader
      :title="$t('Support doctors')"
      :text="$t('From here you can manage system support doctors')"
    />
    <Filter
      :fillters="fillters"
        ...
        >
          ...
          <td>
            <VListItem @click="show(item.id)">
              <VListItemTitle>{{ item.doctor.name }}</VListItemTitle>
            </VListItem>
          </td>
          <td>
            {{ item.comments }}
